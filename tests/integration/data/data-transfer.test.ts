...
const suite = useMongoIntegrationSuite('data transfer');
const SOURCE_DATABASE = `${suite.databaseName}_source`;
const TARGET_DATABASE = `${suite.databaseName}_target`;
...
describe('streaming data transfer', () => {
  ...
  beforeAll(async () => {
    client = await suite.newClient(await suite.standaloneUri);
  ...
  it('copies filtered BSON documents without modifying the source', async () => {
    const sourceCollection = client.db(SOURCE_DATABASE).collection('orders');
    ...
    const opened = source.open(client, SOURCE_DATABASE, 'orders', "{ status: 'active' }");
    for (;;) {
    ...
    expect(await client.db(TARGET_DATABASE).collection('orders_copy').countDocuments()).toBe(2);
  ...
  it('streams RFC 4180 CSV rows, including BOM and multiline cells', async () => {
    ...
    expect(await client.db(TARGET_DATABASE).collection('csv_products').countDocuments()).toBe(2);
    await manager.dispose();
  });

  it('applies skip, replace and merge conflict policies only on the target', async () => {
    const target = client.db(TARGET_DATABASE).collection('conflicts');
    ...
    const skipped = await writeTransferBatch(client, {
      database: TARGET_DATABASE, collection: 'conflicts', documentsEjson: [duplicate, fresh],
    ...
    await writeTransferBatch(client, {
      database: TARGET_DATABASE, collection: 'conflicts', documentsEjson: [duplicate],
