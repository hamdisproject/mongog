...
import { getReplSetUri, getStandaloneUri, newClient, stopAll } from './helpers/mongo.js';

const DATABASE = 'mongog_phase5';
...
describe('Phase 5 administration operations', () => {
  ...
  beforeAll(async () => {
    [standalone, replicaSet] = await Promise.all([
      newClient(await getStandaloneUri()),
      newClient(await getReplSetUri()),
    ]);
