...
const suite = useMongoIntegrationSuite('sampling');
...
describe('schema sampling', () => {
  ...
  beforeAll(async () => {
    ...
    const coll = client.db(DATABASE).collection('mixed');
    const docs = [];
    for (let i = 0; i < 100; i++) {
      docs.push({
        _id: new ObjectId(),
  ...
  it('infers paths, types, presence and array element types', async () => {
    const result = await sampleSchema(client.db(DATABASE), 'mixed', { sampleSize: 100 });
  ...
  it('honours inference path bounds', async () => {
    const result = await sampleSchema(client.db(DATABASE), 'mixed', {
      sampleSize: 20,
      maxPaths: 4,
      maxDepth: 1,
    });
  ...
  it('returns a cacheable empty inferred schema for an empty collection', async () => {
    const result = await sampleSchema(client.db(DATABASE), 'empty', { sampleSize: 50 });
