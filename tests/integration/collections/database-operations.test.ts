...
const suite = useMongoIntegrationSuite('database operations');
...
describe('database create and rename operations', () => {
  ...
  beforeAll(async () => {
    [standalone, replicaSet] = await Promise.all([
      suite.newClient(suite.standaloneUri),
      suite.newClient(suite.replicaSetUri),
    ]);
  }, 120_000);
  ...
  it.each([
    ...
    const database = `${suite.databaseName}_create_${topology.replaceAll(' ', '_')}`;
