...
const suite = useMongoIntegrationSuite('cursor paging');
...
describe('cursor pagination', () => {
  ...
  beforeAll(async () => {
    const uri = await suite.standaloneUri;
    client = await suite.newClient(uri);
    registry = new CursorRegistry();
    engine = new ExecutionEngine(registry);

    const coll = client.db(DATABASE).collection('big');
  ...
  it('idle TTL expires cursors automatically', async () => {
    ...
    const cursor = client.db(DATABASE).collection('big').find({});
    const id = ttlRegistry.register(cursor, { connectionId: 'paging' }, `${DATABASE}.big`);
  ...
  it('fetches a full BSON document only through its retained value handle', async () => {
    ...
    const collection = client.db(DATABASE).collection('full_values');
  ...
  it('closeAllForOwner cleans cursors on tab close/disconnect', async () => {
    ...
    const a = client.db(DATABASE).collection('big').find({});
    const b = client.db(DATABASE).collection('big').find({});
