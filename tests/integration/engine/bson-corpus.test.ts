...
const suite = useMongoIntegrationSuite('bson corpus');
...
describe('BSON corpus round-trip through the engine', () => {
  ...
  beforeAll(async () => {
    ...
    await client.db(DATABASE).collection('corpus').deleteMany({});
    await client.db(DATABASE).collection('corpus').insertOne(corpusDoc);
  });
