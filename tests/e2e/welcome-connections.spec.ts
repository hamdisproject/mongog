...
test('Welcome, Connections, and Collection Query provide the complete lifecycle', async () => {
  ...
  await page.getByRole('button', { name: 'What’s New in 1.2.16' }).click();
  ...
  await expect(page.getByText('Installed v1.2.16')).toBeVisible();
  await expect(page.locator('[data-release-version="1.2.16"]')).toContainText('Latest');
  ...
  await expect(page.getByTestId('about-updates-settings')).toContainText('MongoG 1.2.16');
...
test('update available is surfaced in the sidebar after consent is declined', async () => {
  ...
  await expect(page.getByTitle(/MongoG version /)).toContainText('v1.2.16');
...
test('packaged updater follows the platform download policy with a real generic feed', async () => {
  ...
  try {
    ...
    if (!automaticDownload) page.once('dialog', (dialog) => dialog.dismiss());
    await inspectUpdateConfiguration(application!, cachePath);
...
async function inspectUpdateConfiguration(application: ElectronApplication, expectedCachePath: string): Promise<void> {
