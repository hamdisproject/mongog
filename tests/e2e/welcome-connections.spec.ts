...
test('Welcome, Connections, and Collection Query provide the complete lifecycle', async () => {
  ...
  await page.getByRole('button', { name: 'What’s New in 1.2.3' }).click();
  ...
  await expect(page.getByText('Installed v1.2.3')).toBeVisible();
  await expect(page.locator('[data-release-version="1.2.3"]')).toContainText('Latest');
  ...
  await expect(page.getByTestId('about-updates-settings')).toContainText('MongoG 1.2.3');
