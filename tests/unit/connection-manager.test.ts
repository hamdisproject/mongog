...
import type { ConnectionDraftRequest, TestConnectionResult } from '../../src/shared/domain/index.js';
...
function createManager(tester: ConnectionTester | null = null): ConnectionManager {
  const db = Database.openOrCreate(dbPath);
  return new ConnectionManager(db, mockSupervisor, secretStore, tester);
}

function draftRequest(input: Partial<ConnectionDraftRequest> = {}): ConnectionDraftRequest {
  return {
    draft: {
      name: 'Draft', groupId: null, uri: 'mongodb://localhost:27017',
      defaultDatabase: 'app', readOnly: false, color: '#4ec9b0',
